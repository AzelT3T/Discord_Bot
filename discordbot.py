...
def check_with_openai(question):
    ...
    print(question_list)
    prompt = f"{question}という質問がありましたが、これは {question_list} のいずれかの質問と一致しますか？回答は今から指定する''で囲まれた中の()の中のみを変えて返答してください。絶対に私が指定した形式を外れないでください。余計な文章を入れないように。返答の際には''と()は削除してください。'(yesもしくはno),(question_listのどの質問と一致するか)'。回答例を示します('こんにちは。あなたは誰ですか？', 'どういう機能がありますか？'がquestion_listにある場合)。回答例:yes,どういう機能がありますか？"
